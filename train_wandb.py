...
def log_metrics(iter_num, train_loss, val_loss):
    """
...
def train(prepared_train_labels, train_images_folder, num_refinement_stages, base_lr, batch_size, batches_per_iter,
    ...
    train_loader = DataLoader(
        ...
        num_workers=0 if device == 'mps' else num_workers,  # Set to 0 for MPS
        pin_memory=True
        )
    ...
    if checkpoint_path:
        checkpoint = torch.load(checkpoint_path, map_location=device)  # Change here
    ...
    net = DataParallel(net).to(device)  # Change here
    net.train()
    
    wandb.init(
        project="sita-lightweight-openpose",
        name="log_e_numerator",
        # wandb_log = True,
        )  # Replace with your project and run names
    ...
    for epochId in range(current_epoch, 280):
        ...
        for batch_data in train_loader:
            ...
            images = batch_data['image'].to(device)  # Change here
            keypoint_masks = batch_data['keypoint_mask'].to(device)  # Change here
            paf_masks = batch_data['paf_mask'].to(device)  # Change here
            keypoint_maps = batch_data['keypoint_maps'].to(device)  # Change here
            paf_maps = batch_data['paf_maps'].to(device)  # Change here
            ...
            if num_iter % log_after == 0:
                ...
                for loss_idx in range(len(total_losses) // 2):
                    print('\n'.join(['stage{}_pafs_loss:     {}', 'stage{}_heatmaps_loss: {}']).format(
                        loss_idx + 1, total_losses[loss_idx * 2 + 1] / log_after, #the average PAF loss for the current stage.
                        loss_idx + 1, total_losses[loss_idx * 2] / log_after)) #the average heatmaps loss for the current stage.
                    
                log_metrics(num_iter, np.log(total_losses[0] / log_after), np.log(total_losses[1] / log_after))
                # Reset total losses
                for loss_idx in range(len(total_losses)):
                    total_losses[loss_idx] = 0

                #total_lossez[0]: train loss. This is the average heatmap loss calculated over the last log_after iterations.
                # It provides insight into how well the model is learning to predict keypoints.
                #total_lossez[1]: validation loss. This is the average paf loss calculated over the last log_after iterations.
                # which indicates how well the model is learning to predict the relationships between keypoints.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch-size', type=int, default=16, help='batch size') #default is 80
    ...
    parser.add_argument('--log-after', type=int, default=50, help='number of iterations to print train loss')
    ...
    parser.add_argument('--checkpoint-after', type=int, default=3000,
