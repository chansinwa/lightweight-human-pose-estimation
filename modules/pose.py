...
def pose_init(img, init_frame, webcam_kpts_list=None):
        ...
        if "normalized_coords" in kpt and kpt["coords"] != [-1, -1]:
            ...
            for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):
                ...
                if kpt_a_id in scaled_kpt and kpt_b_id in scaled_kpt:
                    cv2.line(
                        img, (int(x_a), int(y_a)), (int(x_b), int(y_b)), [255, 0, 0], 80
                    )
            ...
            if webcam_kpts_list is not None:
                ...
                print("\nkpt_id:", kpt_id)
                print("kpt['ref_scaled_coords']:", kpt["ref_scaled_coords"])
                print("webcam_kpt_corrds:", webcam_kpt_corrds)
                
                if webcam_kpt_corrds != [-1, -1]:
                    abs_distance = np.linalg.norm(np.array(kpt["ref_scaled_coords"]) - np.array(webcam_kpt_corrds))
                    # abs_distance = Pose.calculate_distance(
                    #     kpt["ref_scaled_coords"], webcam_kpt_corrds
                    # )
                ...
                if abs_distance <= 80 and abs_distance != 0:
...
class Pose:
    ...
    def draw(self, img):
        ...
        for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):
            ...
            if global_kpt_a_id != -1 and global_kpt_b_id != -1:
                x_b, y_b = self.keypoints[kpt_b_id]
                cv2.line(
                    img, (int(x_a), int(y_a)), (int(x_b), int(y_b)), Pose.line_color, 5
                )
        ...
        if ref_kpts_list is None:
            ...
            for part_id in range(len(BODY_PARTS_PAF_IDS) - 2):
                ...
                if global_kpt_a_id != -1:
                    x_a, y_a = self.keypoints[kpt_a_id]
                    cv2.circle(
                        img, (int(x_a), int(y_a)), 6, Pose.skeleton_overlay_color, -1
                    )
                kpt_b_id = BODY_PARTS_KPT_IDS[part_id][1]
                global_kpt_b_id = self.keypoints[kpt_b_id, 0]
                if global_kpt_b_id != -1:
                    x_b, y_b = self.keypoints[kpt_b_id]
                    cv2.circle(
                        img, (int(x_b), int(y_b)), 6, Pose.skeleton_overlay_color, -1
                    )
                if global_kpt_a_id != -1 and global_kpt_b_id != -1:
                    cv2.line(
                        img,
            ...
            if len(ref_kpts_list) > 0:
                ...
                for kpt in ref_kpts_list[0]:
                    if "normalized_coords" in kpt and kpt["coords"] != [-1, -1]:
                        ...
                        if webcam_kpts_list is not None:
                            webcam_kpt_corrds = webcam_kpts_list[kpt_id]["coords"]
                            abs_distance = self.calculate_distance(
                                kpt["ref_scaled_coords"], webcam_kpt_corrds
                            )
                            ...
                            if abs_distance <= 80:
                                ...
                                circle_colors.append(
                                    (0, 255, 0)
                                )  # Green color for correct posture
                            else:
                                circle_coordinates.append((int(x), int(y)))
                                circle_colors.append(
                                    (0, 0, 255)
                                )  # Red color for incorrect posture
    ...
    def draw_angle_sector(self, img, center, line1, line2, radius=50):
        ...
        # Draw the arc
        cv2.ellipse(
            img, center, (radius, radius), 0, start_angle, end_angle, (255, 0, 0), 2
        )
    ...
    def draw_angles(self, img):
        ...
        ## Right Arm
        if (
            self.is_valid_point(RShoulder)
            and self.is_valid_point(RElbow)
            and self.is_valid_point(RWrist)
        ):
            if r_arm_angle is not None:
                # self.draw_angle_sector(img, tuple(RElbow), RShoulder, RWrist)
                self.draw_interior_sector(
                    img, tuple(RElbow), tuple(RShoulder), tuple(RWrist), "right"
                )
                # cv2.putText(img, f"{r_arm_angle:.2f}", tuple(RElbow), cv2.FONT_HERSHEY_SIMPLEX, font_scale_r, (0, 0, 0), 2)
                self.draw_text_with_outline(
                    img, f"{r_arm_angle:.1f}", tuple(RElbow), font_scale_r, 1
                )

        ## Left Arm
        if (
            self.is_valid_point(LShoulder)
            and self.is_valid_point(LElbow)
            and self.is_valid_point(LWrist)
        ):
            if l_arm_angle is not None:
                # self.draw_angle_sector(img, tuple(LElbow), LShoulder, LWrist)
                self.draw_interior_sector(
                    img, tuple(LElbow), tuple(LShoulder), tuple(LWrist), "left"
                )
                # cv2.putText(img, f"{l_arm_angle:.2f}", tuple(LElbow), cv2.FONT_HERSHEY_SIMPLEX, font_scale_l, (0, 0, 0), 2)
                self.draw_text_with_outline(
                    img, f"{l_arm_angle:.1f}", tuple(LElbow), font_scale_l, 1
                )

        ## Right Shoulder
        if (
            self.is_valid_point(Neck)
            and self.is_valid_point(RShoulder)
            and self.is_valid_point(RElbow)
        ):
            if r_shoulder_angle is not None:
                ...
                self.draw_text_with_outline(
                    img,
                    f"{r_shoulder_angle:.1f}",
                    tuple(RShoulder),
                    font_scale_r_shoulder,
                    1,
                )
        ...
        if (
            self.is_valid_point(Neck)
            and self.is_valid_point(LShoulder)
            and self.is_valid_point(LElbow)
        ):
            if l_shoulder_angle is not None:
                ...
                self.draw_text_with_outline(
                    img,
                    f"{l_shoulder_angle:.1f}",
                    tuple(LShoulder),
                    font_scale_l_shoulder,
                    1,
                )
        ...
        if (
            self.is_valid_point(Neck)
            and self.is_valid_point(RHip)
            and self.is_valid_point(RAnkle)
        ):
            if r_hip_angle is not None:
                ...
                self.draw_text_with_outline(
                    img, f"{r_hip_angle:.1f}", tuple(RHip), font_scale_r_hip, 1
                )

        ## Left Hip
        if (
            self.is_valid_point(Neck)
            and self.is_valid_point(LHip)
            and self.is_valid_point(LAnkle)
        ):
            if l_hip_angle is not None:
                ...
                self.draw_text_with_outline(
                    img, f"{l_hip_angle:.1f}", tuple(LHip), font_scale_l_hip, 1
                )
    ...
    def draw_interior_sector(self, img, center, pt_a, pt_b, body_part=None):
        ...
        cv2.ellipse(
            img, center, (radius, radius), x_axis_angle, 0, end_angle, arc_color, 2
        )
        ...
        cv2.ellipse(
            img, center, (radius, radius), x_axis_angle, 0, end_angle, sector_color, -1
        )
