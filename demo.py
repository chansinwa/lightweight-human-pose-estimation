...
def run_demo(net, image_provider, height_size, cpu, track, smooth, ref_ckpt_list=None):
    if ref_ckpt_list is not None:
        # Handle the case when ref_ckpt_list is provided
    ...
    for img in image_provider:
        ...
        skeleton_img = np.zeros((img.shape[0], img.shape[1], 4), dtype=np.uint8)  # Add an alpha channel
        skeleton_img[:, :, 3] = 0  # Set alpha channel to 0 for full transparency
        
        # transparent_img = ImageReader('detection/images/transparent.png')
        ...
        # NOT WORK | Blend the transparent skeleton image with the original image
        ...
        for pose in current_poses:
            ...
            if track:
                ...
                console_log(
                    img, {"filename": filename, "frame_num": frame_num}
                )
...
if __name__ == "__main__":
    ...
    run_demo(
        net, frame_provider, args.height_size, args.cpu, args.track, args.smooth, 
    )
